...
module Mutations
  class SignUpMutation < BaseMutation
    ...
    argument :position, String, required: true, validates: { allow_blank: false }
    argument :company, String, required: true, validates: { allow_blank: false }

    field :session_id, String, null: false

    def resolve(email:, password:, first_name:, last_name:, position:, company:)
      user = User.find_by(email:)
      raise GraphQL::ExecutionError, 'User already exists' if user.present? && user.invite_status == 'joined'

      new_user = User.signup(email:, password:, first_name:, last_name:, position:, company:)
